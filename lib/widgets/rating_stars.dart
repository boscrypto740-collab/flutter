...
class RatingStars extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: List.generate(5, (i) => Icon(
      ...
      color: const Color(0xFFFAC775), size: size)));
}
...
class _InteractiveRatingStarsState extends State<InteractiveRatingStars> {
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: List.generate(5, (i) => GestureDetector(
      onTap: () { setState(() => _rating = i+1.0); widget.onRatingChanged(_rating); },
      child: Padding(padding: const EdgeInsets.symmetric(horizontal: 2),
        child: Icon(i < _rating ? Icons.star_rounded : Icons.star_outline_rounded,
          color: i < _rating ? const Color(0xFFFAC775) : AppColors.textMuted, size: 32)))));
}
